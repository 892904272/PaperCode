...
###############################  DDPG  ####################################
class DDPG(object):
    def __init__(self, a_dim, s_dim, lstm_dim,a_bound,n_time):
        ...
        self.memory = np.zeros((MEMORY_CAPACITY, s_dim + 1 + 1
                                + s_dim + lstm_dim + lstm_dim))
        ...
        self.lstm_s = tf.placeholder(tf.float32, [None, n_lstm_step,lstm_dim], 'lstm1_s')  # 输入
        self.S_ = tf.placeholder(tf.float32, [None, s_dim], 's_')
        self.lstm_s_ = tf.placeholder(tf.float32, [None, n_lstm_step, lstm_dim], 'lstm1_s_')  # 输入
