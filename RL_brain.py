...
class DeepQNetwork:
    ...
    def __init__(self,
                 n_actions,                  # the number of actions行动的数量
                 n_features,
                 n_lstm_features,
                 n_time,
                 learning_rate = 0.01,
                 reward_decay = 0.9,
                 e_greedy = 0.99,
                 replace_target_iter = 200,  # each 200 steps, update target net每200步，更新目标网络
                 memory_size = 500,  # maximum of memory最大内存
                 batch_size=32,
        ...
        # lstm
        self.N_lstm = N_lstm
        self.n_lstm_step = n_lstm_step       # step_size in lstm 以lstm为单位的步长
        ...
        # replace the parameters in target net替换目标网络中的参数
        ...
        self.lstm_history = deque(maxlen=self.n_lstm_step)# 创建一个双端队列
    ...
    def _build_net(self):
        ...
        def build_layers(s,lstm_s,c_names, n_l1, n_lstm, w_initializer, b_initializer):
            ...
            with tf.variable_scope('l0'):
                lstm_dnn = tf.contrib.rnn.GRUCell(n_lstm)#n_lstm lstm中神经元数量
                lstm_dnn.zero_state(self.batch_size, tf.float32)#状态初始化，batch_size 一次训练选取的样本数
        ...
        with tf.variable_scope('eval_net'):
            ...
            # c_names(collections_names), will be used when update target_net更新目标网络时将使用（集合名称）
    ...
    def store_transition(self, s, lstm_s,  a, r, s_, lstm_s_):
        ...
        # if memory overflows, replace old memory with new one如果内存溢出，请用新内存替换旧内存
    ...
    def choose_action(self, observation):
        ...
        # observation = observation[np.newaxis, :] #改变维度，转化为一个矩阵，进行运算
        #
        ...
        #     print("self.lstm_history")
        #     print(self.lstm_history)
        #     print("lstm_observation")
        #     print(lstm_observation)
        #
        ...
        #
        #     action = np.argmax(actions_value) #np.argmax返回最大值的索引值，即哪个行为
        #
        # else:
        #
    ...
    def learn(self):
        ...
        #从内存np.hstack中随机选取[batch_size]内存（（s[a，r]，s_，lstm_，lstm_））
        ...
        # 获取q_next（从目标网络）（到q_目标）和q_eval（从评估网络）
        # 最小化（目标-评估）^2
        # q_目标=奖励+伽马*q_下一步
        # 在bacth_内存的大小中
        # q_next，给定批处理中的下一个状态，q_next中的下一个q_是什么
        # q_eval4next，给定批处理的下一个状态，q_eval的q_eval4next是什么
        ...
        # q_eval，给定批处理的当前状态，q_eval的q_eval是什么
        ...
        # self.s和self.q_目标都属于eval_
        # 输入self.s和self.q_目标、输出self._训练、self.loss（以最小化差距）
        # self.sess.run：给定输入（提要），输出所需元素
        _, self.cost = self.sess.run([self._train_op, self.loss],
                                     feed_dict={self.s: batch_memory[:, :self.n_features],
                                                self.lstm_s: lstm_batch_memory[:, :, :self.n_lstm_state],
                                                self.q_target: q_target})  #:是在赋值
        # print("------cost---")
        # print(self.cost)
    ...
    def do_store_reward(self, episode, time, reward):
        ...
        print("reward:" + str(reward) + "episode:" + str(episode) + "time:" + str(time))
    ...
    def do_store_action(self,episode,time, action):
        ...
        print("action:" + str(action) + "episode:" + str(episode) + "time:" + str(time))
    ...
    def do_store_delay(self, episode, time, delay):
        ...
        print("delay:" + str(delay) + "episode:" + str(episode) + "time:" + str(time))
